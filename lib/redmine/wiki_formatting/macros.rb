...
module Redmine
  module WikiFormatting
    module Macros
      ...
      desc "pagebreak."
      macro :pagebreak do |obj, args, text|
        content_tag('br','',{:style=>'page-break-after: always;'})        
      end
      ...
      macro :thumbnail do |obj, args|
        ...
        filename = args.first
        id, filename = filename.split('/') if filename.include? '/'
        ...
        attachment = id ? Attachment.find(id) : Attachment.latest_attach(obj.attachments, filename)
        if obj && obj.respond_to?(:attachments) && attachment 
