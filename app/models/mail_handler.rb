  ...
  def add_attachments(obj)	
    if email.attachments && email.attachments.any?
      email.attachments.each do |attachment|
		...
		logger.info "ATTACHMENT #{attachment.filename} to ##{obj.id}"		
  ...
  def get_project_from_receiver_addresses
    ...
    [:to, :cc, :bcc].each do |field|
      ...
      header.field.addrs.each do |addr|
        if addr.domain.to_s.casecmp(domain)==0 
		  recipient,domain = addr.local.to_s.split("@")
		  if project = Project.find_by_identifier(recipient)
			return project
		  end
		  if addr.local.to_s =~ /\A#{local}\+([^+]+)\z/
